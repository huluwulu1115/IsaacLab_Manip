...
def main():
    ...
    while step < args_cli.max_steps:
        ...
        student.train()
        student_action = student.act(proprio, rgbd, deterministic=False)
        ...
        # Loss (student learns to match teacher)
        loss = criterion(student_action, teacher_action).mean()
        ...
        reward_tracker.update(rewards, dones)
        ...
        if step % log_interval == 0:
            ...
            if ep_stats["episodes_completed"] > 0:
                print(f"Step {step:6d} | Loss: {mean_loss:.4f} | "
                      f"EpisodeReward: {ep_stats['episode_reward_mean']:.2f} "
                      f"(±{ep_stats['episode_reward_std']:.2f}, {ep_stats['episodes_completed']} eps) | Beta: {beta:.2f}")
            else:
