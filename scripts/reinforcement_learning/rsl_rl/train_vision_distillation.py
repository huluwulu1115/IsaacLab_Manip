...
parser.add_argument(
    "--beta_start_decay", type=int, default=None, help="Step to start beta decay (default: use config value)"
)
parser.add_argument(
    "--beta_end_decay", type=int, default=None, help="Step to end beta decay (default: use config value)"
)
parser.add_argument(
    "--use_kl_loss", action="store_true", default=False, help="Use KL divergence loss instead of DEXTRAH weighted MSE"
)
parser.add_argument(
    "--target_std",
    type=float,
...
parser.add_argument(
    "--video", action="store_true", default=False, help="Record training videos (will slow down training)"
)
parser.add_argument("--video_interval", type=int, default=10000, help="Interval between video recordings (in steps)")
parser.add_argument(
    "--video_length", type=int, default=150, help="Length of each recorded video (in steps, ~1 episode at 14.3Hz)"
)
...
def evaluate_on_training_env(
    student: nn.Module, env, device: str, num_episodes: int, step: int, teacher: Optional[nn.Module] = None
):
    ...
    student.eval()
    eval_step = 0
    max_eval_steps = num_episodes * 200  # Safety limit (~143 steps/episode at 14.3Hz)
                ...
                for idx in done_indices:
                    if episodes_completed < num_episodes:
                        ...
                        if episodes_completed % 10 == 0:
                            print(
                                f"  Episode {episodes_completed}/{num_episodes}: " f"R={ep_reward:.2f}, Len={ep_length}"
                            )
...
def main():
    ...
    print(
        f"  - Beta schedule: {beta_start:,} → {beta_end:,} steps (linear decay, {beta_end-beta_start:,} steps transition)"
    )
    ...
    run_name = (
        f"{loss_type_str}_" f"b{beta_start//1000}k-{beta_end//1000}k_" f"std{args_cli.target_std:.1f}_" f"{et_str}"
    )
    if video_str:
    ...
    print(
        f"  Teacher baseline: Success={initial_results['success_rate']:.1f}%, R/step={initial_results['mean_r_per_step']:.2f}"
    )
    ...
    while step < args_cli.max_steps:
        ...
        if step % eval_interval == 0 and step > 0:
            ...
            print(
                f"  Evaluation @ {step:,}: Success={eval_results['success_rate']:.1f}%, R/step={eval_results['mean_r_per_step']:.2f}\n"
            )
            ...
            if args_cli.video:
                ...
                upload_videos_to_wandb(
                    video_dir=video_dir, uploaded_videos=uploaded_videos, eval_interval=eval_interval, verbose=True
                )
    ...
    if args_cli.video and video_dir and video_dir.exists():
        ...
        num_uploaded = upload_videos_to_wandb(
            video_dir=video_dir, uploaded_videos=uploaded_videos, fallback_step=step, verbose=True
        )
